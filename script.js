...
const hiddenNetworkCategories = new Set();

function getActiveCategories() {
  return new Set(activeCategories);
}

function getHiddenNetworkCategories() {
  return new Set(hiddenNetworkCategories);
}
...
function isCategoryHiddenInNetwork(category) {
  if (!category || category === "system") return false;
  return hiddenNetworkCategories.has(category);
}
...
function toggleCategoryNetworkVisibility(category) {
  ...
  if (hiddenNetworkCategories.has(category)) {
    hiddenNetworkCategories.delete(category);
  } else {
    hiddenNetworkCategories.add(category);
  }
...
let galleryLightboxCaptionEl = null;
let galleryLightboxCounterEl = null;

function getGalleryLightbox() {
  if (galleryLightboxEl) return galleryLightboxEl;

  galleryLightboxEl = document.createElement("div");
  galleryLightboxEl.className = "gallery-lightbox";
  galleryLightboxEl.setAttribute("role", "dialog");
  galleryLightboxEl.setAttribute("aria-modal", "true");
  galleryLightboxEl.setAttribute("aria-label", "Visionneuse photos");
  galleryLightboxEl.hidden = true;
  galleryLightboxEl.innerHTML = `
    <div class="gallery-lightbox__backdrop" data-gallery-close></div>
    <div class="gallery-lightbox__panel">
      <button type="button" class="gallery-lightbox__close" data-gallery-close aria-label="Fermer la visionneuse">✕</button>
      <button type="button" class="gallery-lightbox__nav gallery-lightbox__nav--prev" data-gallery-prev aria-label="Photo précédente">‹</button>
      <figure class="gallery-lightbox__figure">
        <img class="gallery-lightbox__image" alt="" />
        <figcaption class="gallery-lightbox__caption"></figcaption>
      </figure>
      <button type="button" class="gallery-lightbox__nav gallery-lightbox__nav--next" data-gallery-next aria-label="Photo suivante">›</button>
      <div class="gallery-lightbox__counter" aria-live="polite"></div>
    </div>
  `;

  galleryLightboxImageEl = galleryLightboxEl.querySelector(".gallery-lightbox__image");
  galleryLightboxCaptionEl = galleryLightboxEl.querySelector(".gallery-lightbox__caption");
  galleryLightboxCounterEl = galleryLightboxEl.querySelector(".gallery-lightbox__counter");

  galleryLightboxEl.addEventListener("click", (event) => {
    if (event.target.closest("[data-gallery-close]")) closeGalleryLightbox();
    if (event.target.closest("[data-gallery-prev]")) showGalleryLightboxImage(activeGalleryIndex - 1);
    if (event.target.closest("[data-gallery-next]")) showGalleryLightboxImage(activeGalleryIndex + 1);
  });

  document.body.appendChild(galleryLightboxEl);
  return galleryLightboxEl;
}
...
function showGalleryLightboxImage(index) {
  ...
  if (galleryLightboxImageEl) {
    galleryLightboxImageEl.src = item.src;
    galleryLightboxImageEl.alt = item.alt || "Photo liée à l'événement";
  }

  if (galleryLightboxCaptionEl) {
    const captionParts = [item.caption, item.alt].filter(Boolean);
    galleryLightboxCaptionEl.textContent = captionParts[0] || "";
    galleryLightboxCaptionEl.hidden = !galleryLightboxCaptionEl.textContent;
  }

  if (galleryLightboxCounterEl) {
    galleryLightboxCounterEl.textContent = `${activeGalleryIndex + 1} / ${activeGalleryItems.length}`;
  }
}

function openGalleryLightbox(items, startIndex = 0) {
  activeGalleryItems = Array.isArray(items) ? items : [];
  if (!activeGalleryItems.length) return;
  ...
  showGalleryLightboxImage(startIndex);
...
function closeGalleryLightbox() {
  if (!galleryLightboxEl) return;

  galleryLightboxEl.classList.remove("is-open");
  galleryLightboxEl.hidden = true;
  document.body.classList.remove("gallery-lightbox-open");
  if (galleryLightboxImageEl) galleryLightboxImageEl.removeAttribute("src");
}

function handleGalleryLightboxKeydown(event) {
  if (!galleryLightboxEl || galleryLightboxEl.hidden) return;
  ...
  if (event.key === "Escape") {
    ...
    event.preventDefault();
    showGalleryLightboxImage(activeGalleryIndex - 1);
...
` : ""}
        ...
        <div class="event-gallery">
          ${galleryItems.map((item, index) => `
            <figure class="event-gallery__item">
              <button type="button" class="event-gallery__button" data-gallery-index="${index}" aria-label="Agrandir la photo ${index + 1} sur ${galleryItems.length}">
  ...
  const galleryButtons = drawerContentEl.querySelectorAll("[data-gallery-index]");
  galleryButtons.forEach((button) => {
    button.addEventListener("click", () => {
      const index = Number(button.dataset.galleryIndex || 0);
      openGalleryLightbox(galleryItems, index);
    });
  });
}
...
function buildNetworkCategoryToggleMarkup(label, isVisible, isHidden) {
  const checkboxStateLabel = isHidden
    ? `Catégorie masquée totalement dans le réseau : réactive d'abord l'œil pour modifier cet état.`
    : `Rendre ${escapeHtml(label)} normale ou transparente`;
      ...
      <span class="network-category-toggle__content">
        <span class="network-category-toggle__checkbox">
          <input
            ...
            ${isVisible ? "checked" : ""}
            ${isHidden ? "disabled" : ""}
            aria-label="${checkboxStateLabel}"
          />
        </span>
        <span class="network-category-toggle__text">${escapeHtml(label)}</span>
        <span class="network-category-toggle__spacer"></span>
        <button
          type="button"
          class="network-category-toggle__eye ${isHidden ? "is-hidden" : "is-visible"}"
          aria-label="${isHidden ? "Réafficher" : "Masquer totalement"} ${escapeHtml(label)} dans le réseau"
          title="${isHidden ? "Réafficher dans le réseau" : "Masquer totalement dans le réseau"}"
        >
...
function renderNetworkCategories() {
  ...
  availableCategories.forEach((category) => {
    ...
    const markerColor = CATEGORY_COLORS[category] || "#94a3b8";
    const isVisible = isCategoryVisible(category);
    const isHidden = isCategoryHiddenInNetwork(category);

    const item = document.createElement("div");
    item.className = `network-category-toggle ${isVisible ? "is-active" : "is-inactive"} ${isHidden ? "is-hidden" : "is-shown"}`;
    item.dataset.category = category;
    item.style.setProperty("--category-accent", markerColor);
    item.innerHTML = buildNetworkCategoryToggleMarkup(label, isVisible, isHidden);
    ...
    const eyeButton = item.querySelector('.network-category-toggle__eye');
    eyeButton?.addEventListener("click", (e) => {
      e.preventDefault();
      e.stopPropagation();
      toggleCategoryNetworkVisibility(category);
    });

    item.addEventListener("click", (e) => {
      if (e.target.closest('.network-category-toggle__eye')) return;
      if (isHidden) return;
      toggleCategoryVisibility(category);
    });
...
function buildNetworkGraphData(options = {}) {
  const visibleCategoriesSet = options.visibleCategoriesSet || getActiveCategories();
  const hiddenCategoriesSet = options.hiddenCategoriesSet || getHiddenNetworkCategories();
  const normalized = normalizeEvents(events)
    .filter((event) => event.id !== "TODAY")
    .filter((event) => !hiddenCategoriesSet.has(event.category));
  const eventLookupMaps = buildEventLookupMaps(normalized);

  const nodeMap = new Map();
  const linkMap = new Map();

  const registerNodeCategory = (node, category) => {
    if (!node || !category || category === "system") return;
    if (!node.associatedCategories) {
      node.associatedCategories = new Set();
    }
    node.associatedCategories.add(category);
  };

  const ensureNode = (id, payload) => {
    if (!nodeMap.has(id)) {
      nodeMap.set(id, { id, degree: 0, associatedCategories: new Set(), ...payload });
    }
    const node = nodeMap.get(id);
    if (payload?.category) registerNodeCategory(node, payload.category);
    if (Array.isArray(payload?.categories)) {
      payload.categories.forEach((category) => registerNodeCategory(node, category));
    }
    return node;
  };
  ...
  const ensureCountryNode = (country, category = null) => {
  ...
  const linkStepTree = (steps, event, parentNodeId) => {
    ...
    steps.forEach((step, index) => {
      ...
      const stepCategory = getMapCategory(step, event.category);
      ...
      const countryNodeId = ensureCountryNode(step.country, stepCategory);
      if (countryNodeId) {
  ...
  normalized.forEach((event) => {
    ...
    if (hasSteps) {
      ...
      const countryNodeId = ensureCountryNode(event.country, event.category);
      if (countryNodeId) {
  ...
  const nodes = Array.from(nodeMap.values()).map((node) => {
    ...
    const categories = Array.from(node.associatedCategories || []);
    const isMuted = categories.length > 0 && categories.every((category) => !visibleCategoriesSet.has(category));
    return {
      ...node,
  ...
  const links = Array.from(linkMap.values()).map((link) => {
    ...
    return {
      ...link,
      muted: Boolean(sourceNode?.muted || targetNode?.muted)
    };
  });
...
function renderKnowledgeGraph(visibleCategoriesSet = getActiveCategories(), hiddenCategoriesSet = getHiddenNetworkCategories()) {
  ...
  const data = buildNetworkGraphData({ visibleCategoriesSet, hiddenCategoriesSet });
  networkGraphEl.innerHTML = "";

  if (!data.nodes.length || !data.links.length) {
    networkGraphEl.innerHTML = '<div class="network-panel__empty">Aucune relation disponible pour les catégories actuellement affichées.</div>';
    networkGraphState = null;
    return;
  }
